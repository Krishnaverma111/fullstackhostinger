...
import { 
  ChevronUp, ShoppingCart, Star, ArrowLeft, ArrowRight, 
  CheckCircle, ListFilter, Search, Zap, Info, Sparkles
} from 'lucide-react';
...
const Seed = ({ addToCart }) => {
  ...
  useEffect(() => {
    const fetchSeeds = async () => {
      try {
        ...
        // ✅ STRICT FILTER (ONLY SEEDS)
        const seedOnlyData = data.filter(
          item => item.category?.trim().toLowerCase() === "seeds"
        );
        ...
        const formattedData = seedOnlyData.map(item => ({
          ...
          img: item.image,
          price: Number(item.price),
          oldPrice: Number(item.price) + 200,
          type: item.category,
          rating: "4.5",
          isHot: false
        }));
  ...
  if (loading) return <div className="text-center mt-40">Loading Seeds...</div>;

  return (
    <div className="bg-[#f8fafc] min-h-screen pt-28 px-6">

      {/* BACK */}
      <Link to="/" className="flex items-center gap-2 text-green-600 font-bold mb-6">
        <ArrowLeft size={16}/> Back to Home
      </Link>

      {/* HEADER */}
      <div className="flex justify-between items-center mb-12">
        <h1 className="text-5xl font-black">Seeds <span className="text-green-600">Vault</span></h1>
      ...
      {/* GRID */}
      {filteredSeeds.length === 0 ? (
        <div className="text-center">No Seeds Found</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">

          {filteredSeeds.map((item) => (
            <div key={item.id} className="bg-white p-5 rounded-3xl shadow hover:shadow-xl transition">

              <div className="h-56 overflow-hidden rounded-2xl">
                <img src={item.img} className="w-full h-full object-cover"/>
              </div>

              <h3 className="font-bold mt-4 text-lg">{item.name}</h3>
              ...
              <button
                ...
                className="w-full mt-4 bg-black text-white py-3 rounded-xl hover:bg-green-600"
              >
                Add To Cart
              </button>
