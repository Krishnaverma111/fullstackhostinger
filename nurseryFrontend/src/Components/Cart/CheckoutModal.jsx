...
const CheckoutModal = ({ cartItems, totalAmount, isOpen, onClose, clearCart }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // ✅ response capture kiya — orderId isi se milega
            const response = await placeOrder(orderData);

            // ✅ Payment page ke liye checkoutData localStorage mein save
            localStorage.setItem('checkoutData', JSON.stringify({
                orderId: response?.orderId || response?.data?.orderId || response?._id || '',
                customer: {
            ...
            // ✅ seedha payment page pe redirect — alert() hata diya
            window.location.href = '/payment';

        } catch (err) {
            console.error("Order Error:", err);
            alert("Order save nahi ho paya: " + err.message);
            // ❌ error case mein clearCart/onClose/redirect NAHI karna — user dobara try kare
        } finally {
            setLoading(false);
        }
    };
