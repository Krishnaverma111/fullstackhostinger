...
const Navbar = ({ cartItems = [] }) => {
  ...
  // Auto rotate placeholder text every 2s (only jab input khali ho)
  useEffect(() => {
    const interval = setInterval(() => {
      setPlaceholderIndex((prev) => (prev + 1) % navLinks.length);
    }, 2000);
  ...
  // Filtered suggestions based on searchQuery
  ...
  // Navigate to matching page (exact match first, fallback partial match)
  const goToSearchResult = (query) => {
    const q = query.trim().toLowerCase();
    if (!q) return;

    const exactMatch = navLinks.find((link) => link.name.toLowerCase() === q);
    const partialMatch = navLinks.find((link) => link.name.toLowerCase().includes(q));
    const match = exactMatch || partialMatch;

    if (match) {
      navigate(match.path);
    } else {
      // koi match nahi mila -> generic search results page
      ...
      <header className="fixed w-full z-50 top-0 mt-8 lg:mt-0 ">
        ...
        <nav className={`w-full transition-all duration-500 ${isScrolled ? 'bg-white/90 backdrop-blur-xl py-2 shadow-lg' : 'bg-white py-4'}`}>
          <div className="max-w-[1920px] mx-auto px-4 lg:px-12 flex justify-between items-center gap-4">
            ...
            {/* Search Bar */}
            <div ref={searchRef} className="hidden lg:flex flex-1 max-w-xl relative group">
                <Search
                  size={18}
                  onClick={() => goToSearchResult(searchQuery)}
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-green-600 transition-colors cursor-pointer hover:text-green-600 z-10"
                />
                <input 
                  type="text"
                  value={searchQuery}
                  ...
                  placeholder={`Search for ${navLinks[placeholderIndex].name}...`}
                  className="w-full bg-gray-50 border border-gray-100 rounded-full py-3.5 pl-12 pr-4 text-sm focus:bg-white focus:ring-4 focus:ring-green-500/5 transition-all outline-none" 
                />
                ...
                <AnimatePresence>
                  {showSuggestions && filteredSuggestions.length > 0 && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full left-0 w-full bg-white shadow-xl rounded-2xl border border-gray-100 mt-2 overflow-hidden z-20"
                    >
                      {filteredSuggestions.map((link) => (
                        <button
                          key={link.name}
                          onClick={() => handleSuggestionClick(link)}
                          className="w-full text-left px-6 py-3 text-sm font-semibold text-gray-600 hover:bg-green-50 hover:text-green-700 transition-colors flex items-center justify-between"
                        >
                          <span>{link.name}</span>
            ...
            <motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} transition={{type:'spring', damping:25}} className="fixed top-0 right-0 h-full w-[85%] max-w-sm bg-white z-[70] shadow-2xl xl:hidden flex flex-col">
              ...
              <div className="px-6 pt-6 relative">
                <Search
                  size={18}
                  onClick={() => goToSearchResult(searchQuery)}
                  className="absolute left-10 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer z-10"
                />
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  onKeyDown={handleSearch}
                  placeholder={`Search for ${navLinks[placeholderIndex].name}...`}
                  className="w-full bg-gray-50 border border-gray-100 rounded-full py-3.5 pl-12 pr-4 text-sm outline-none"
                />
                {showSuggestions && filteredSuggestions.length > 0 && (
                  <div className="mt-2 bg-white shadow-lg rounded-2xl border border-gray-100 overflow-hidden">
                    {filteredSuggestions.map((link) => (
                      <button
                        key={link.name}
                        onClick={() => handleSuggestionClick(link)}
                        className="w-full text-left px-6 py-3 text-sm font-semibold text-gray-600 hover:bg-green-50 hover:text-green-700 transition-colors"
                      >
                        {link.name}
                      </button>
                    ))}
