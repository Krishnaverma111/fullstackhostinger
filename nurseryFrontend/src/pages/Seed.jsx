...
const PlantStore = ({ addToCart }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        setFetchedInventory(Array.isArray(items) ? items : []);
      } catch (err) {
  ...
  // Only client-side search + sort; category filtering done by backend
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 mb-20">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {["All", "Indoor", "Outdoor", "Succulent", "Seeds"].map(tab => (
              <button
                key={tab}
            ...
            <motion.div layout className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
              {isLoading ? (
                ...
                <div className="col-span-3 text-center py-32">
                  <p className="text-slate-300 font-black uppercase tracking-widest text-sm">No specimens found.</p>
                </div>
              ) : filtered.map((item) => (
